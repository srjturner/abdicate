...
Context.prototype.getInstance = function(name, callback) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (instance && fac && fac.scope != 'prototype') {
      ...
      return fac.build(self).then(cache(self, name)).then(resolve, (err) => {
...
Context.prototype.getInstances = function(names, callback) {
  ...
  return Promise.all(
      ...
      R.compose(
        R.mapObj((instancesOfSomeName) => { // 4. unpack the array values which have just 1 value
          if (instancesOfSomeName.length == 1) return instancesOfSomeName[0] 
          else return instancesOfSomeName 
        }), 
        R.mapObj(R.chain(R.tail)), // 3. convert the values for each name to arrays of just instances
        R.groupBy(R.head), // 2. group the instances by name, 
...
var registerFunctionAtPath = R.curry(function(context, path, functionAnnotations, functionName) {
  return toFactoryFunction(path, functionName).then((func) => {
    if (func) {
