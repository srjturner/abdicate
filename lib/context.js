...
var registerFunctionsAtPath = R.curry(function(context, path) {
  return readAnnotationsFor(path).then(
    R.compose(Promise.all, R.values, R.mapObjIndexed(registerFunctionAtPath(context, path)))
  )
})
...
var registerFunctionAtPath = R.curry(function(context, path, functionAnnotations, functionName) {
  return toFactoryFunction(path, functionName).then(function(func) {
    if (func) {
      var settings = {name: defaultName(functionName, path, context.rootpaths), scope: 'singleton', async: false, dependencies: []}
...
/**
 * Return a Promise to find the actual function implied by a functionName in the file. 
 */
...
var populate = function(context) {
  return Promise.resolve(buildEdges(context.factories))
        .then(traverseDag(context))
}
