...
var readAnnotations = function(context, filepaths) {
  var registerFunction = function(path, functionAnnotations, functionName) {
  ...
  var registerAnnotatedAtPath = function(path) {
    return readAnnotationsFor(path)
      .then(function(pathAnnotations) {
        R.mapObjIndexed(R.curry(registerFunction)(path), pathAnnotations)          
      })
  }
  return Promise.all(R.compose(R.map(registerAnnotatedAtPath), R.filter(isJavascript))(filepaths))
}
